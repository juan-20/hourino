...
- **The one exception: the Google sign-in button** (`apps/web/src/components/google-sign-in-button.tsx`) follows Google's own branding spec, not this system: exact colors (`#FFFFFF`/`#747775`/`#1F1F1F` light, `#131314`/`#8E918F`/`#E3E3E3` dark), Roboto Medium 14px (self-hosted via `@fontsource/roboto`), the official multicolor G mark, 40px height, 4px corners and Google's localized labels. Google requires it; don't restyle it to the riso tokens.
