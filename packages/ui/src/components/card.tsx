...
function Card({
	...
	return (
		<div
			className={cn(
				"group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-lg border-2 border-border bg-card py-(--card-spacing) text-card-foreground text-xs/relaxed shadow-brutal [--card-spacing:--spacing(4)] has-[>img:first-child]:pt-0 has-data-[slot=card-footer]:pb-0 data-[size=sm]:has-data-[slot=card-footer]:pb-0 data-[size=sm]:[--card-spacing:--spacing(3)] *:[img:first-child]:rounded-t-md *:[img:last-child]:rounded-b-md",
				className
			)}
			data-size={size}
			data-slot="card"
